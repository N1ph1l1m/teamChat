...
export function MessageMenu({ menu, setEmojiWindow ,  emojiWindow ,  onEmojiSelect}){
    if(!menu) return null;

    const handleEmojiSelect = (emoji) => {
      if (onEmojiSelect) {
        onEmojiSelect(emoji.native); // Передаем выбранный смайлик
      }
    };
  ...
  <ul className={styles.moreMenu} >
      ...
      <li onClick={setEmojiWindow}><MdOutlineEmojiEmotions  color="rgb(117, 117, 117)"  size ="20" style={{marginRight:"4px"}}/> <span>Реакция</span></li>
      </ul>
      {emojiWindow ? (<div className={styles.wrapEmoji}>
      <Picker
              data={data}
              theme={"ligth"}
