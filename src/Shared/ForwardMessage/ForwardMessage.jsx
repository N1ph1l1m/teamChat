...
export default function ForwardMessage({ forwardMessage, photoData }) {
  const forwwardBy = () => {
    if (
    ...
    return (
      <>
        {firstForward.map((forward, index) => (
          <h4 className={styles.forwardMessageTitle} key={index}>
            Переслано от {forward.forwarded_by.username}
          </h4>
        ))}
      </>
    );
      ...
      <>
        <div className={styles.forwardMessageHeader}>
          ...
          <div className={styles.forwardHeaderWrapTitle}>
            <HeaderName username={forward.original_message.user.username} />
            <span className={styles.forwardTime}>
              {dateText(forward.original_message.created_at_formatted)} в{" "}
              {forward.original_message.created_at_formatted.substring(11, 17)}
            </span>
          </div>
        </div>
      </>
            ...
            {Array.isArray(forwardMessage.forwarded_messages) &&
              ...
              forwardMessage.forwarded_messages.map((forward, index) => (
                <div key={index} className={styles.forwardMessageContentWrap}>
                  {header(forward)}
                  <MessagePhoto
                    photos={forward.original_message.images.map(
                      (img) => img.image
                    )}
                    modalPhoto={() => {}}
                    photoData={photoData}
                  />
                  <MessageDocuments
                    documents={forward.original_message.documents}
                  />
